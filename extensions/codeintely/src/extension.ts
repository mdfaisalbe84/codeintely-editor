...
export function activate(context: vscode.ExtensionContext) {
  ...
  // First-ever activation on this profile: the "codeintely" container is
  // now declared under viewsContainers.secondarySidebar (package.json) —
  // a real, public VS Code contribution point (confirmed against
  // viewsExtensionPoint.ts: 'secondarySidebar' -> ViewContainerLocation.AuxiliaryBar)
  // that places it in the right-side Secondary Side Bar directly,
  // affecting only this container — unlike the earlier, wrong approach of
  // setting workbench.panel.defaultLocation globally, which would have
  // also moved Terminal/Output/Problems. This reveal just ensures it's
  // actually opened/focused the first time, since a fresh Secondary Side
  // Bar can start collapsed. Relies on globalState persisting across
  // restarts, which requires a working @vscode/sqlite3 native binary for
  // this Electron build.
